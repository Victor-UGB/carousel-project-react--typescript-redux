...
const ShareModal = () => {
    ...
    const share = (textToShare:string, network:string) => {
        ...
        if (network === "whatsapp"){
            ...
            try{
                window.open(`twitter://share?url=${encodeURIComponent(url)}&text=${encodeURIComponent(textToCopy)}, '', left=0,top=0,width=550,height=450,personalbar=0,toolbar=0,scrollbars=0,resizable=0`)
            }catch(error){
                console.log(error)
            }
        }
